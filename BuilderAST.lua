...
function LLX.Class(name)
  ...
  local r = tryF(L, {"GetClass","ResolveClass","LookupClass","Class"}, name)
  if r then return { kind="class", name=name, data=r, src="LuaLib" } end
  local s = tryF(L, {"GetSignature"}, name)
  if type(s) == "string" and s:match("%S") then
    return { kind="class", name=name, sig=s, src="LuaLib" }
  end
  local supers = L.SUPER or L.Supers or L.ClassTree
  if type(supers) == "table" and supers[name] then
    return { kind="class", name=name, supers=supers[name], src="LuaLib" }
  end
  if type(L.Classes) == "table" and L.Classes[name] then
    return { kind="class", name=name, data=L.Classes[name], src="LuaLib" }
  end
  return nil
end
...
function LLX.Method(cls, nm)
  ...
  local r = tryF(L, {"GetMethod","ResolveMethod","LookupMethod"}, cls, nm)
  if r then return { kind="method", class=cls, name=nm, data=r, src="LuaLib" } end
  local s = tryF(L, {"GetSignature"}, cls .. ":" .. nm)
  if type(s) == "string" and s:match("%S") then
    return { kind="method", class=cls, name=nm, sig=s, src="LuaLib" }
  end
  return nil
end
...
function LLX.Prop(cls, nm)
  local L = M._ll
  if not L or not cls or not nm then return nil end
  local r = tryF(L, {"GetProperty","ResolveProperty","LookupProperty"}, cls, nm)
  if r then return { kind="property", class=cls, name=nm, data=r, src="LuaLib" } end
  local s = tryF(L, {"GetSignature"}, cls .. "." .. nm)
  if type(s) == "string" and s:match("%S") then
    return { kind="property", class=cls, name=nm, sig=s, src="LuaLib" }
  end
  return nil
end
...
function LLX.Event(cls, nm)
  local L = M._ll
  if not L or not cls or not nm then return nil end
  local r = tryF(L, {"GetEvent","ResolveEvent","LookupEvent"}, cls, nm)
  if r then return { kind="event", class=cls, name=nm, data=r, src="LuaLib" } end
  local s = tryF(L, {"GetSignature"}, cls .. "." .. nm)
  if type(s) == "string" and s:match("%S") and s:lower():match("event") then
    return { kind="event", class=cls, name=nm, sig=s, src="LuaLib" }
  end
  return nil
end
...
function LLX.Enum(path)
  ...
  local r = tryF(L, {"ResolveEnumFull","ResolveEnum","GetEnum","LookupEnum"}, path)
  if r then return { kind="enum", enum=path, data=r, src="LuaLib" } end
  return nil
end
...
function LLX.EnumByValue(enumName, val)
  ...
  local r = tryF(L, {"ResolveEnumValue","GetEnumValue","EnumFromValue"}, enumName, val)
  if r then return { kind="enum", enum=enumName, val=val, data=r, src="LuaLib" } end
  return nil
end
...
local function inferCls(e)
  ...
  if not c.colon and c.object and c.object.type == "Name"
     and c.property == "new" and a and a.type == "StringLiteral" then
    return a.val, "ctor"
  end
end
...
local function scanBinds(tree)
  ...
  M.walk(tree, function(n)
    local tg, vs
    if n.type == "LocalStatement" then tg, vs = n.names, n.values
    elseif n.type == "AssignmentStatement" then tg, vs = n.targets, n.values end
    if tg and vs then
...
local function enrichCall(n, b)
  ...
  if not bd then return end
  local info = (c.lua and c.lua.sig) and c.lua or LLX.Method(bd.class, c.property)
  if info then
    n.lua = {
      kind = "call",
      cls  = bd.class,
